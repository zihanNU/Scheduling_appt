...
def similarity_cal_ml(df ):
    df['ref' ] = CONFIG._similarity_ref
    histload_feature = {'weight':df.hist_weight /CONFIG._weight_scale ,
                        'length':df.hist_miles /CONFIG._miles_scale
 }
    histload_df=pd.DataFrame(histload_feature).astype(np.float32)
    newload_feature = {'weight':df.new_weight /CONFIG._weight_scale ,
                        'length':df.new_miles /CONFIG._miles_scale}
...
def similarity_cal_rules(df):
    df['ref'] = CONFIG._similarity_ref
    histload_feature = {'weight': df.hist_weight / CONFIG._weight_scale,
                        'pallet': df.hist_pallet / CONFIG._weight_scale,
                        'length': df.hist_miles / CONFIG._miles_scale
                        }
    histload_df = pd.DataFrame(histload_feature).astype(np.float32)
    newload_feature = {'weight': df.new_weight / CONFIG._weight_scale,
                       'pallet': df.new_pallet / CONFIG._weight_scale,
                       'length': df.new_miles / CONFIG._miles_scale}
    newload_df = pd.DataFrame(newload_feature).astype(np.float32)
    sim_row = ((histload_df.values / norm(histload_df.values, axis=1)[:, np.newaxis]) * (
            newload_df.values / norm(newload_df.values, axis=1)[:, np.newaxis])).sum(axis=1)
...
def scheduler_ml(newloads_df,histloads_df):
    ...
    histloadfeatures =['LoadID' ,'PU_Hour' ,'DO_Hour','PU_Transit_Minute','Miles','TotalWeight','PU_Facility' ,'CustomerID' ,'DO_Facility'
                        , 'PU_Bucket', 'DO_Bucket' ]
    newhistjoin_df2 =histloads_df[histloadfeatures]
    newhistjoin_df2.columns =['histloadID' ,'PU_Hour' ,'DO_Hour','PU_Transit_Minute','hist_miles','hist_weight' ,'PU_Facility' ,'hist_customer'
                            ,'DO_Facility', 'PU_Bucket', 'DO_Bucket' ]
    hour_ml_df = pd.DataFrame(columns=['LoadID','PU_Hour_ml','DO_Hour_ml'] )
    try:
        facilityjoin_df = newhistjoin_df1.merge(newhistjoin_df2,\
                                     left_on=["new_PF","new_DF"],\
...
def scheduler_rule(newloads_df,histloads_df):
    ...
    newhistjoin_df2 =histloads_df[histloadfeatures]
    newhistjoin_df2.columns =['histloadID' ,'PU_Hour' ,'DO_Hour' ,'hist_miles','hist_weight' ,'hist_PF' ,'hist_customer'
                            ,'hist_DF' ,'hist_PC' ,'hist_DC' ]
    hour_ml_df = pd.DataFrame(columns=['LoadID','PU_Hour_ml','DO_Hour_ml'] )
    try:
        facilityjoin_df = newhistjoin_df1.merge(newhistjoin_df2, left_on=["PU_Facility", "DO_Facility"],\
                                            right_on=["PU_Facility", "DO_Facility"],suffixes=('_new', '_hist'), how='inner')
    except Exception as e:
        LOGGER.exception('unexpected error while merging on Facility PU and DO: {0}'.format(e))

    try:
        originjoin_df = newhistjoin_df1.merge(newhistjoin_df2, left_on=["PU_Facility" ],\
                                            right_on=["PU_Facility" ],suffixes=('_new', '_hist'), how='inner')
    except Exception as e:
        LOGGER.exception('unexpected error while merging on Facility PU: {0}'.format(e))
    try:
        destjoin_df = newhistjoin_df1.merge(newhistjoin_df2, left_on=["DO_Facility" ],\
                                            right_on=["DO_Facility" ],suffixes=('_new', '_hist'), how='inner')
    except Exception as e:
        LOGGER.exception('unexpected error while merging on Facility DO: {0}'.format(e))
