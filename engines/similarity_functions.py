...
def similarity_check(newloads_df, histloads_df):
    ...
    joinOrig_df = newhistjoin_df1.merge(newhistjoin_df2, left_on=["new_PF"],
                                               right_on=["PU_Facility"], how='inner')
    joinOrig_df.drop(['new_PF', 'new_Orig_Cluster', 'new_Dest_Cluster',  'hist_Orig_Cluster', 'hist_Dest_Cluster',
                       ],
                   axis=1, inplace=True)

    joinDest_df = newhistjoin_df1.merge(newhistjoin_df2, left_on=["new_PF"],
                                         right_on=["PU_Facility"], how='inner')
    ...
    joinTravel_df = newhistjoin_df1.merge(newhistjoin_df2, left_on=['new_Orig_Cluster', 'new_Dest_Cluster'],
                                             right_on=['hist_Orig_Cluster', 'hist_Dest_Cluster'], how='inner')
    joinTravel_df.drop(['new_PF'], axis=1, inplace=True)

    joinArea_df = newhistjoin_df1.merge(newhistjoin_df2, on=['OriginClusterID', 'DestinationClusterID'], how='inner')

    if joinAll_df.shape[0] > 0:  # Type A and D
    ...
    if joinTravel_df.shape[0] > 0:
        facility_travel_df = similarity_cal_travel(joinTravel_df)
