def feasibility_check(load_df, facility_df):
    ...
    load_df['DO_DOW'] = load_df['do_scheduletime'].dt.dayofweek
    load_df['PU_DOW'] = load_df['pu_scheduletime'].dt.dayofweek
    load_df['PUopen_hour'] = load_df.apply(lambda x: facility_df.loc[x['PU_Facility'], Weekday_Mapper[x['PU_DOW']]+'Open'])
    load_df['PUclose_start']
    load_df['DOopen_hour'] = pd.Timestamp('now')
    load_df['DOclose_hour']
